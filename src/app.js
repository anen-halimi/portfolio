...
function projectCard(p) {
  return '<article class="project-card" data-project="'+p.id+'">'+visual(p)+'<div class="project-body"><div class="project-type"><span>'+escapeHTML(p.kind)+'</span><span>'+p.number+'</span></div><h3><button class="title-case" data-case="'+p.id+'">'+p.title+'</button></h3><p>'+escapeHTML(p.description)+'</p>'+tags(p.tags)+'<div class="project-actions"><button class="case-button" data-case="'+p.id+'" aria-label="Lire l’étude de cas : '+escapeHTML(p.shortTitle)+'">Lire l’étude de cas <span>↗</span></button><a href="'+base.linkedin+'?trk=portfolio-access" target="_blank" rel="noopener noreferrer">Demander l’accès ↗</a></div></div></article>';
}
function render() {
